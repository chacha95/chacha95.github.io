...
퍼셉트론은 인공신경망(Artificial Neural Network)의 한 종류료서, 1957년에 코넬 항공 연구소의 Rosenblatt에의해 만들어졌습니다. 이것은 가장 간단한 feedfoward 네트워크로 볼 수 있습니다.

feedforward는 앞으로 순차적으로 feed를 주는 시스템을 말합니다.



Perceptron은 input 벡터 [x1, x2]가 있고 이를 weight 벡터 [w1, w2]와 element-wise multiplication 한뒤 모든 elements를 더해주고, activation function(sigmoid)를 통해 나오는 값이 출력 값이 됩니다.
...
이러한 MLP 구조에서 hidden layer를 깊게(많이) 쌓은 네트워크 모델이 바로 deeplearning 모델 입니다.
