...
특히나 차원이란 기저 벡터의 수로 결정되며, 벡터는 물리학, computer science, 수학에서 의미하는 바가 다릅니다. 물리학에선 속력과 방향을 가진 것으로 표현되고, computer science에서는 순서를 가진 순차 리스트 이며, 수학에선 이 두개를 포괄하는 개념으로 이용됩니다. 관련해서 [자세히 소개하는 영상](https://www.youtube.com/watch?v=fNk_zzaMoSs&list=PLZHQObOWTQDPD3MizzM2xVFitgF8hE_ab)을 보시는걸 추천합니다.

일단 우리가 볼 것은 computer science에서 보는 관점인 순차 리스트로서의 벡터입니다. 

Computer science에서 이미지를 표현하는 방식에 대해 봅시다. 사진의 경우를 예로 생각해 보겠습니다. 아래 두 장의 차원이 다른 사진을 보시겠습니다. 

왼쪽 그림은 크기가 120 x 80 (즉 9,600 차원), 오른쪽 그림은 1200 x 800 (960,000 차원)으로 크기가 백배 차이가 납니다. 차원이 큰 사진이 용량도 100 배 더 크고, 정보도 훨씬 고밀도로 저장되어 있습니다.
...
데이터의 차원이 증가할수록 해당 공간의 크기(부피)가 기하급수적으로 증가하기 때문에 동일한 개수의 데이터의 밀도는 차원이 증가할수록 급속도록 sparse(희소, 희박)해집니다. 
...
<center><img src="https://user-images.githubusercontent.com/31475037/64427493-50ebca00-d0ec-11e9-981e-419102c83846.png" width="60%"></center>
...
그렇다면 어떻게 고차원의 공간을 저차원으로 projection 할 수 있을까요?
...
PCA 는 데이터의 분산(variance)를 최대한 보존하면서 서로 직교하는 principal axis를 찾아, 고차원 공간의 표본들을 선형 연관성이 없는 저 차원 공간으로 변환하는 기법입니다. 즉 원 데이터의 분산을 최대한 보존하는 새 기저입니다.

그렇다면 어떻게 principal axis를 찾을까요? 

다음의 예시를 통해 알아봅시다. 먼저 Random values를 원형으로 뿌려 놓았을 때를 가정해봅시다.

> Random values를 원형으로 뿌려 놓았을 때

<center><img src="https://user-images.githubusercontent.com/31475037/64518271-d2ca3600-d32c-11e9-9567-eb50ac0b5c3a.PNG" width="50%"></center>
이 Random values의 점들에 대해 다음 행렬을 통해 선형 변환을 해봅시다.
...
$$
Covariance \ Matrix(X, Y) = \begin{bmatrix}
Cov(X, X) & Cov(X, Y)\\
Cov(Y, X) & Cov(Y, Y)\\
\end{bmatrix}
$$
...
$$
Cov(X, Y)는 \ X의 \ 편차와 \ Y의 \ 편차를\ 곱한것의 \ 평균
$$
...
<center><img src="https://raw.githubusercontent.com/angeloyeo/angeloyeo.github.io/master/pics/2019-07-27_PCA/pics_mtx1.png" width="70%"></center>
> 행렬에 의해 선형 변환 된 후

<center><img src="https://user-images.githubusercontent.com/31475037/64518272-d362cc80-d32c-11e9-8723-66b652027704.PNG" width="50%"></center>
Eigenvector의 의미를 잘 생각해보면, eigenvector는 행렬이 벡터에 작용하는 principal axis의 방향을 나타내므로 covariance 행렬의 principal axis는 데이터가 어떤 방향으로 퍼져 있는지 잘 나타내준다고 할 수 있습니다.

Eigenvalue는 eigenvector 방향으로 얼마만큼의 크기로 vector space가 늘려지는 지를 말합니다. 따라서 eigenvalue가 큰 순서대로 Eigenvector를 정렬하면 결과적으로는 중요한 순서대로 principal axis를 구하는 것이 됩니다.


> Covariance Matrix의 principal axis

<center><img src="https://raw.githubusercontent.com/angeloyeo/angeloyeo.github.io/master/pics/2019-07-27_PCA/pics_eigen.png" width="50%"></center>
...
마지막으로 노이즈 제거의 의미는 k 개의 주성분만을 이용해서 데이터를 복원함으로써 의미 없는 노이즈 부분을 날린다는 의미입니다.(k 값이 작아질수록 의미없는 데이터)
