...
gRPC는 Google에서 개발한 RPC 시스템입니다. 전송을 위해 HTTP 2.0을 사용하고 IDL(Interface Definition language)로 protocol buffer를 사용합니다.
...
이렇다 보니, HOL(Head Of Line) Blocking-특정응답지연, RTT(Round Trip TIme) 증가, 헤비한 Header구조라는 문제점들을 가지고 있습니다. 이러한 부분을 HTTP2에선 개선하였습니다. Multiplexed Streams(한 커넥션에 여러개의 메세지를 동시에 주고 받을 수 있음), Stream Prioritization(요청 리소스간 의존관계를 설정), Server Push(HTML문서상에 필요한 리소스를 클라이언트 요청없이 보내줄 수 있음), Header Compression(Header 정보를 HPACK 압축방식을 이용하여 압축전송)을 사용해 성능을 향상시켰습니다.
...
RESTful API + JSON 조합으로 많이 사용됩니다.
...
Protocol buffers는 구조화(structured)된 데이터를 직렬화(serialization)하기 위한 프로토콜로 XML보다 작고 빠르고 간단합니다. XML 파일처럼 .proto 파일에 protocol buffer message 타입을 정의합니다. 
...
SSL / TLS를 사용하여 서버를 인증하고 클라이언트와 서버간에 교환되는 모든 데이터를 암호화합니다. HTTP 2.0을 사용하여 성능이 뛰어나고 확장 가능한 API를 지원합니다. 요청 및 응답의 크기를 줄이기 위해 헤더 압축을 사용합니다.
