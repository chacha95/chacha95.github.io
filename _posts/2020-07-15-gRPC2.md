---
layout: post
title: protocol buffer(proto3) 가이드
tags: [Backend]
use_math: true
---
...
프로토콜 버퍼는 structured data(구조화 데이터)를 seriailize(직렬화)하기위한 Google의 언어 중립적, 플랫폼 중립적이며 확장 가능한 메커니즘입니다. 
...
이 field numbers는 메세지 binary format에서 field를 식별하는데 사용됩니다. 즉 field number가 key가 되고 field type이 value가 되는 해시 구조를 지닙니다.
...
Fields를 완전히 제거하거나 주석 처리하여 메세지 type을 업데이트하면, 향후 사용자는 메세지 type을 업데이트 할 때 field number를 재사용 할 수 있습니다. 나중에 동일한 .proto의 old version을 로드하면 문제가 발생할 수 있습니다. 이러한 일이 발생하지 않도록 하는 한 가지 방법은, 삭제 된 field의 field number를 지정하는 것입니다. 

### What's Generated From Your .proto?

.proto 파일을 protocol buffer compiler에 의해 complied되면, 메세지s는 serializing되어 output stream으로 나가고 parsing되어 input stream으로 들어옵니다. 대표적인 3가지는 다음과 같습니다.
