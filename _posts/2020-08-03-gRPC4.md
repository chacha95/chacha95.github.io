---
layout: post
title: gRPC4 - gRPC Architecture(구조)
tags: [Backend]
use_math: true
---
...
브라우저에서 gRPC를 사용하기 위해선 브라우저가 proto 파일을 가지고 있어야 하기에 쓰기가 사실상 불가능합니다.

### 클라이언트 업데이트시 .proto 파일을 업데이트 해줘야함

만일 서버의 proto 파일에 변화가 생긴다면 클라이언트의 proto 파일도 업데이트가 필요합니다. 그렇지 않다면 오류를 발생할 것입니다.

<br>

# 결론

요약하자면 gRPC를 사용하면 service를 .proto 파일로 한 번 정의하고, gRPC 지원 언어로 클라이언트와 서버를 만들 수 있으며, 이는 다양한 환경에서 실행될 수 있습니다. 다른 언어와 다른 환경 간의 통신을 gRPC를 통해 처리합니다. 또한 효율적인 직렬화, 간단한 IDL인 protocol buffer를 사용해 성능적 이점을 얻을 수 있습니다.
