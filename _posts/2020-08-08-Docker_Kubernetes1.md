...
컨테이너는 기본적으로 etho0과 lo 네트워크 인터페이스를 가짐

컨테이너는 내부 IP를 순차적으로 할당, 컨테이너가 재시작될 때마다 변경

veth 인터페이스는 docker 엔진에 의해 자동 생성

<br>

## Bridge network

사용자가 정의한 브릿지를 생성해 각 컨테이너에 연결하는 네트워크 구조로 컨테이너는 연결된 브릿지를 통해 외부와 통신 가능

브릿지 네트워크로 설정되어 있고, 별칭으로 묶인 컨테이너들은 docker 내부 DNS를가 감지해 정확한 IP 반환
