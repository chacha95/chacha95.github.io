---
layout: post
title: 딥러닝을 위한 kubernetes4 - 애플리케이션 실행
tags: [Backend]
use_math: true
---
...
Kubectl 명령어를 실행하면 kube-apiserver에 REST API로 yaml로 작성된 애플리케이션 디스크립션이 전송됩니다.
...
위의 모든 과정에서 유저가 컨테이너를 직접 생성하지 않습니다. 단지 앱 디스크립터에 기술할 뿐이죠. 유저가 kubectl을 이용해 run 명령을 내리면, 레플리케이션컨트롤러를 생성, 레플리케이션컨트롤러가 실제 pod을 생성합니다. 즉, 앱 디스크립터를 잘 기술하고, 도커 허브에 이미지만 올바르게 업로드하면 쿠버네티스가 모든 작업을 처리해줍니다.

<br>

## 쿠버네티스 오브젝트 기술하기

쿠버네티스에서 오브젝트를 생성할 때, 오브젝트에 대한 기본 정보와 더불어, 의도한 상태(desired state)를 기술한 오브젝트 spec을 적어야합니다.

**대부분의 경우 정보를 .yaml 파일 작성됩니다.**

**kubectl**은 kube-apiserver에 yaml 파일 전송시, JSON 형식으로 정보를 변환시켜 전송합니다.
...
쿠버네티스에서 가장 중요한 것은 **desired state** 라는 개념입니다. Desired state란 관리자가 바라는 환경을 의미하며, 구체적으론 애플리케이션의 종류, 스펙, 몇 번 포트로 서비스하기를 원하는지 등을 의미합니다.
...
실행중인 앱은 레플리케이션컨트롤러에 의해 모니터링되고, 실행되며 서비스를 통해 외부에 노출됩니다. 쿠버네티스의 주요 이점 중 하나는 간단하게 배포를 확장 가능, 즉 pod 개수를 간단하게 늘릴 수 있다는 점입니다.
