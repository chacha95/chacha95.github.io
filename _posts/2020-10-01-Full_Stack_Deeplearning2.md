...
File System은 복잡도가 낮지만, 데이터를 각각의 파일 단위로 저장,처리하기에 데이터에 대한 **종속성**, **중복성**, **무결성** 문제가 생기게 되었고 이를 해결하기 위해 RDBMS(Relational Database Management System)가 등장했습니다..
...
- table를 정의하고 이 table에 맞지 않는 데이터는 저장되지 않음
...
기존 RDBMS는 데이터의 무결성, 중복성등을 보장해주는 대신, table를 정의하고 정의한 table에 맞게만 데이터를 저장할 수 있습니다. 또한 RDBMS의 경우 정규화 작업을 통해 무결성, 중복성을 없애주어야 하는데, 이러한 정규화 과정을 거치게 되면 table가 여러개로 쪼개지게 되고 각각의 table끼리 foreign key(외래키)를 통해 relation(관계)를 맺어주어야 하며, 이로인해 CRUD 성능이 떨어지게 됩니다. 특히나 read 성능이 떨어지게되 기존 3NF 까지 정규화 하던것을 1NF나 2NF까지만 하는 **반정규화**를 하기 시작합니다. 즉, read 성능을 높이기 위해서 데이터의 무결성과 중복성을 포기하는 것이죠.
...
그러던 와중 기존 RDBMS처럼 table에 정의가 힘든 경우(SNS의 게시글)나 혹은 높은 CRUD 성능이 필요한 경우들이 생겨나기 시작했습니다. 이러한 요구에 발맞춰 느슨한 table를 지향하며 대량의 분산 데이터를 CRUD하는데 특화된 NoSQL(Not Only SQL or NoSQL)이 나왔습니다.
...
Data lake를 통해 저장된 데이터들은 data Engineer에 의해 정제되어 저장됩니다. 

- **level 0**: 데이터는 버전 관리없이 file system or DB에 있습니다.
- **level 1**: 데이터는 학습 시간에 모든 스냅 샷을 저장하여 버전이 지정됩니다.
- **level 2**: 데이터는 자산과 코드의 조합으로 버전이 지정됩니다.
- **level 3**: 데이터 버전 관리를위한 특수 솔루션이 필요합니다.

그렇다면 정제된 데이터들은 어떻게 관리할까요?
