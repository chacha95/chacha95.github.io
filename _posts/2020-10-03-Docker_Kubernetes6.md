...
Proxy 기술은 서비스의 트래픽을 네트워크 단에서 통제할 수 있게 하고, Client의 요구에 따라 proxy단에서 라우팅서비스도 가능케 할 수 있습니다. 그러나 TCP기반(L4 layer)의 proxy로는 한계가 있습니다.

MSA가 커지면서 서비스들은 네트워크를 통해 더 많은 통신이 필요했습니다. 이러한 상황에서 핵심 네트워크 프로토콜은 HTTP 등의 L7 layer에서 동작하는 프로토콜입니다.

> 커져가는 시스템

![](https://www.abhishek-tiwari.com/assets/images/Network-of-Microservices.jpg)

이렇게 커져가는 MSA에서 L3, L4기반의 프록시들만으론 모든 요청을 처리하기 어려워졌고, **L7기능을 갖춘 프록시**가 나타났습니다.
...
서킷 브레이커 패턴처럼 프록시를 넣음

중간에 타임 아웃 같은게 일어나면 끊어 버림
...
envoy proxy를 컨트롤 할 수 있게 만든게 Istio(이스티오)
...
Istio에서의 통신은 사이드카로 배치된 **경량화된 L7 layer proxy**를 사용합니다.
...
Service mesh의 기본 아키텍처는 서비스 간 직접 호출 대신 앞에서 언급한 경량화된 사이드카 패턴의 프록시를 배치하여 통신합니다. 이런 service mesh 기술의 대표적인 구현체가 Istio입니다. 

Data plane의 main proxy로 envoy, control plane로 istio 사용
...
3. **Pilot**은 트래픽 규칙과 구성을 Envoy Proxy에 배포
...
![](https://user-images.githubusercontent.com/31475037/95040469-b1de3b00-070e-11eb-976f-35b93a45ce43.png)
