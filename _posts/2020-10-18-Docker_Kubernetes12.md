...
auto scailing, networking, status check, 서버 구성의 복잡성을 캡슐화하여 GPU 자동 확장, 0으로 확장, 카나리아 롤아웃과 같은 최첨단 서비스 기능을 ML 배포에 제공합니다. 예측, 사전 처리, 사후 처리 및 설명 가능성을 즉시 제공하여 프로덕션 ML 추론 서버에 대한 간단하고 플러그 가능하며 완전한 스토리를 지원합니다.
