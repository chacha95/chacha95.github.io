---
layout: post
title: 고성능 파이썬 프로그래밍 2 - tips
tags: [Python]
use_math: true
---

Python3에서 쓰이는 여러가지 tip들을 소개합니다.
...
**GIL**은 스레드에서 사용되는 Lock 을 인터프리터 레벨로 확장한 개념인데 여러 스레드가 동시에 실행되는걸 방지합니다. 각 스레드는 다른 스레드에 의해 GIL이  해제되길 기다린 후에야 실행될 수 있습니다. 즉 멀티 스레드로 만들었어도 본질적으로 싱글 스레드로 동작한다는 말입니다.
