...
L3/L4 layer proxy를 쓰면서, L7 layer proxy를 추가적으로 사용합니다. 버퍼링, 속도제한, 라우팅/전달 등과 같은 다양한 작업을 수행합니다.

<br>

## Istio(이스티오)

공식 doc 읽고 다시 만들기 -> 개념이 다 바뀜
...
**Data plane**의 proxy로 envoy, **control plane**으로 Istio를 사용합니다.
...
Citadel을 이용해 보안 TLS 암호화

프록시가 service B를 호출함(pilot이 호출함)
...
이것도 utube 강의 및 공식 doc 읽고 다시 작성 -> 너무 바뀜

오픈소스 serverless 솔루션인 Knative에 대해서 알아보겠습니다.

<br>

## Serverless?

Knative가 무엇인지 알아보기 전에, **serverless**라는 단어를 짚고 넘어갈 필요가 있습니다. Serverless란 단어 의미대로 실제 서버가 존재하지 않는다는 것은 아니고, 개발자가 **서버인프라에 대해 신경쓰지 않아도 된다는** 뜻입니다. 
...
continer만 던지면 알아서 deployment함

이걸 이용해 배포하게 되면 굉장히 쉬워짐

dockerfile을 만들어 던져주고, service.yaml을 정의해 던져주면, 위에 모든걸 다 해줌(kubernetes도 귀찮고 istio도 귀찮음 -> 다 알아서 해줌)

위에서 언급한 AWS lamda 같은 FaaS들은 특정 클라우드 플랫폼에 의존적입니다.(해당 클라우드 서비스에서만 사용 가능) 

이를 해결하기 위해 나온 오픈소스 serverless 솔루션이 **Knative**입니다.

Knative는 다음과 같은 특징을 지닙니다.
