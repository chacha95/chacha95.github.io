...
REST와 HTTP는 동일하지 않습니다. REST는 HTTP 프로토콜을 규격화하는 아키텍쳐 스타일 뿐입니다. 
...
# Flask with multiprocessing

Flask server는 내장 web server를 가지고 있지만, 개발용 이외에는 사용하지 않습니다. 왜냐하면 blocking하게 구현되어 있어, 한번에 하나의 연결만 받을 수 있기 때문입니다.

이러한 문제를 해결하기 위해서 python의 multiprocessing 패키지들을 많이 쓰거나, 간단하게 queue로 구현하고, subprocess를 spawn하기도 합니다.
