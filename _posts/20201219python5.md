...
Flask에서 보여지는 부분과 처리하는 부분을 나누기위해 템플릿 기능을 제공함

템플릿에 사용되는 파일들은 'templates' 디렉토리에 저장됨

css 같은 파일은 static 디렉토리에 저장됨

이 때 Jinja2 템플릿 엔진을 통해 html 문서 내에 코드를 삽입하여 웹 페이지를 동적으로 생성 가능

### jinja2 템플릿

웹 템플릿 엔진은 여러 복잡한 작업을 웹에서 가능케 합니다.

html 코드 안에서 연산작업을 수행, html 안에서 코드 작성 가능케 함

템플릿에서 제공하는 기능으로 html 코드 중복을 줄이고 반복 되는 코드를 간단히 표현

Jinja2 템플릿 엔진을 사용해 html 문서 내에 코드 조각들을 삽입하여 웹 페이지를 동적으로 생성할 수 있음

### html 내 코드 삽입

Jinja2 템플릿 엔진은 statement(구문)은 {% code %}로, Expression(표현)은 '{{code}}'로 감싸서 표시합니다.

기본적인 html 코드는 상속받아서 사용도 가능합니다.
...
Flask는 내장 web server를 가지고 있지만, insecure하고 blocking하게 동작합니다. 그래서 보통 다른 패키지들과 함께 쓰는데, 대표적으론 flask + gunicorn + Nginx 조합입니다. 혹은 multiprocessing 패키지와 큐를 이용해 non-blocking하게 처리하기도 합니다.
