...
**Uvicorn-FastAPI**에서 Uvicorn은 **single process**로만 작동합니다. 이 때문에 병렬성이 부족해져 multi process를 돌려야 하는 상황(병렬처리)에 대처할 수 없습니다. 때문에 **Gunicorn**을 통해 process를 동시에 여러개를 돌려 병렬성을 확보해야합니다.
